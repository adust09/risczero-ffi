...
Lean4とRISC Zero zkVMは、根本的に異なる目的のために設計されている。

| | Lean4 | RISC Zero zkVM |
|--|-------|----------------|
| **設計目的** | 汎用プログラミング言語 + 定理証明器 | 証明可能な計算の実行環境 |
| **実行モデル** | 豊かなランタイム環境を前提 | 最小限の決定論的環境 |
| **メモリ管理** | ヒープ + 参照カウントGC | 固定メモリ、動的アロケーションなし |
| **I/O** | ファイル、ネットワーク、標準入出力 | `env::read` / `env::commit` のみ |
| **並行性** | マルチスレッド、タスク並列 | 厳密にシングルスレッド |
| **数値型** | 任意精度整数（Nat）が基本 | 固定幅整数のみ実用的 |

**Lean4は「プログラマが便利に使える環境」を、zkVMは「全ての計算ステップを暗号学的に証明できる環境」を目指している。この設計思想の違いが互換性を阻んでいる。**

---

### 本質的課題1: 「証明されたコード」と「実行されるコード」の乖離

このプロジェクトの目的は「Lean4の定理証明 + RISC Zeroのゼロ知識証明 = 二重の信頼性」である。
...
│  Lean4で証明                                             │
│  ┌─────────────┐                                        │
│  │ Spec (Nat)  │ ← 数学的に正しいと証明済み              │
│  └─────────────┘                                        │
│        ↓ 等価証明（native_decide, 計算的）               │
│  ┌─────────────┐                                        │
│  │ Impl(UInt32)│ ← partial使用、終了性未保証            │
│  └─────────────┘                                        │
│        ↓ 手動抽出（人的エラーの余地）                     │
└─────────────────────────────────────────────────────────┘
        ↓
...
│  RISC Zeroで実行                                         │
│  ┌─────────────┐                                        │
│  │ C code      │ ← Lean4の証明が直接適用されない         │
│  └─────────────┘                                        │
└─────────────────────────────────────────────────────────┘
```

**証明の対象（Spec）と実行の対象（C code）の間に複数の変換レイヤーがあり、各レイヤーで信頼性が劣化する。**
...
Leanランタイムには非決定論的な要素が含まれる：
- `malloc`の返すアドレスは実行ごとに異なる可能性
- スレッドスケジューリングの順序
- ハッシュテーブルのイテレーション順序

**zkVMは全ての実行ステップを証明するため、非決定論性を許容できない。**

---

### 本質的課題3: 証明コストの爆発

zkVMでは**全ての命令が証明対象**になる：

```
Leanランタイム初期化:     数百万サイクル
GMP演算1回:              数万サイクル
参照カウント操作:         数十サイクル × 大量の呼び出し

→ 証明生成時間: 数時間〜数日
→ 証明サイズ: 数GB
```

単純な計算でも、ランタイムのオーバーヘッドにより実用的な証明時間・サイズを大幅に超過する。

---

## 技術的課題（本質的課題の帰結）

以下は上記の本質的課題から派生する具体的な技術的障壁である。
...
| アプローチ | 実現性 | 効果 |
|-----------|--------|------|
| Leanランタイムの完全zkVM移植 | ❌ 不可能 | 完全解決 |
| zkVM互換Leanランタイムサブセット | ⚠️ 極めて困難 | 高 |
| Lean→C変換の形式検証 | ⚠️ 困難 | 中 |
| **プリミティブ型に制限** | ✅ 現実的 | 限定的 |
...
ただし、この制限により「証明されたコード」と「実行されるコード」の同一性という本質的課題は完全には解決されず、等価証明と手動抽出プロセスへの信頼が必要となる。
